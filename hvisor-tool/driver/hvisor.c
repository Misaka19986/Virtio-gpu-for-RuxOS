...
static int hvisor_map(struct file *filp, struct vm_area_struct *vma) {
  ...
  if (vma->vm_pgoff == 0) {
    ...
    // vma->vm_flags |= (VM_IO | VM_LOCKED | (VM_DONTEXPAND | VM_DONTDUMP)); Not
    // sure should we add this line.
    ...
    //     pr_err("The physical address to be mapped is not within the reserved
    //     memory\n"); return -EFAULT;
    // }
...
static int __init hvisor_init(void) {
  ...
  if (!node) {
    ...
    pr_info("get virtio_irq: %d", virtio_irq);
